    ...
    <div className="flex flex-column h-auto justify-center gap-4 m-auto p-[1rem] max-w-[600px]">
      ...
      <div className="flex flex-column w-full gap-2 items-center">
        <img
          ...
          className="h-[200px]"
          />
        <p className="text-xl font-bolt text-center">{product.title}</p>
        <p className="text-lg text-center">Description:{product.description}</p>
        <p className="text-lg text-center">${product.price}</p>

        {(Array.isArray(product.reviews) > 0 && product.reviews.length > 0) && (
          <div className="w-full">
            <p className="text-xl font-semibold">Reviews</p>
            <div className="flex flex-col gap-3"/>
              {product.reviews.map((item, index)=>(
                <div key={index} className="border border-gray-200 rounded-xl p-2">
                  ...
                  <p>{item.comment}</p>
                  <p className="text-xs text-gray-500">{new Date(item.date).toLocaleDateString()}
                    </p>
                </div>
              ))}
          </div>
      ...
      <Button color="primary" className="w-[30%] self-center" onClick={()=> router.push("/")}>
        Back
